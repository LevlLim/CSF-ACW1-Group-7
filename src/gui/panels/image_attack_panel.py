"""Attack simulation panel (image): runs the 4 image_attacks against a stego
PNG and shows whether each attack was caught, using the exact same result
each attack function already produces (a real image_decoder.decode_image_file
verdict) — nothing here is a simulated approximation of the verdicts."""
...
class ImageAttackPanel(ctk.CTkFrame): 
    """Run all 4 image attack simulations against a genuine stego PNG."""
    ...
    def __init__(self, master: ctk.CTkBaseClass) -> None:
        ...
        row = 0
        theme.panel_header(
            content, 1, "Image Attack Simulation", "Run tampering, wrong-key, corruption, and wrong-location attacks"
    ...
    def on_run(self) -> None:
        ...
        try:
            tampering = simulate_image_tampering(
                self.stego_path, output_dir / "image_tampered.png",
                lsb_depth=depth, start_secret=secret, media_id=media_id, public_key_pem=public_key_pem,
            )
            wrong_key = simulate_image_wrong_key_verification(
                self.stego_path, lsb_depth=depth, start_secret=secret, media_id=media_id,
            )
            corruption = simulate_image_payload_corruption(
                self.stego_path, output_dir / "image_corrupted.png",
                lsb_depth=depth, start_secret=secret, media_id=media_id, public_key_pem=public_key_pem,
            )
            wrong_location = simulate_image_wrong_start_location(
